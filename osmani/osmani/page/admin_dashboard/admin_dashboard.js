...
class AdminDashboard {
		...
		return $(`
			<div class="summary-cards-row">
				<div class="summary-card primary">
					...
					<div class="card-value">${this.format_currency(data.sales.total_sales)}</div>
				...
				<div class="summary-card success">
					...
					<div class="card-value">${this.format_currency(data.sales.total_recovered)}</div>
				...
				<div class="summary-card warning">
					...
					<div class="card-value">${this.format_currency(data.sales.total_outstanding)}</div>
	...
	create_analytics_sections(data) {
		...
		sections.append(this.create_analytics_card({
			...
			items: [
				{ label: 'Total Sales', value: this.format_currency(data.sales.total_sales), color: 'blue' },
				{ label: 'Recovered', value: this.format_currency(data.sales.total_recovered), color: 'green' },
				{ label: 'Outstanding', value: this.format_currency(data.sales.total_outstanding), color: 'orange' },
				{ label: 'Total Payments', value: this.format_currency(data.payment.total_paid), color: 'purple' }
			]
		}));
		...
		sections.append(this.create_analytics_card({
			...
			icon: 'fa fa-user-tie',
			items: [
				{ label: 'Total Advance', value: this.format_currency(data.employee_advance.total_advance), color: 'purple' },
				{ label: 'Paid Advance', value: this.format_currency(data.employee_advance.total_paid), color: 'green' },
				{ label: 'Total Claims', value: this.format_currency(data.expense_claim.total_claimed), color: 'blue' },
				{ label: 'Reimbursed', value: this.format_currency(data.expense_claim.total_reimbursed), color: 'green' }
			]
		}));
		...
		sections.append(this.create_analytics_card({
			...
			items: [
				...
				{ label: 'Companies', value: data.system.companies, color: 'blue' },
					...
					<tbody>
						${data.top_customers.map((c, idx) => `
							<tr>
								<td>${idx + 1}</td>
								<td>${c.name}</td>
								<td>${this.format_currency(c.total)}</td>
							</tr>
						`).join('')}
					</tbody>
				</table>
					...
					<tbody>
						${data.recent_invoices.map(inv => `
							<tr>
								...
								<td>${this.format_currency(inv.grand_total)}</td>
								<td>${this.format_currency(inv.outstanding_amount)}</td>
